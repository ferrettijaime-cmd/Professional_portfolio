<p align="center">
  <img src="PBI_Sales_Analysis/Images/logo.png" width="150">
</p>

<h1 align="center">Data Analytics Portfolio</h1>

This repository showcases end-to-end Data Analytics, Business Intelligence, Machine Learning, and ETL projects developed using SQL, Python, and Power BI.

The projects focus on solving real-world business problems through data exploration, predictive modeling, customer segmentation, dashboard development, and data pipeline design.

## Skills Demonstrated

- SQL Data Analysis
- Python for Data Analytics
- Machine Learning
- Power BI Dashboard Development
- ETL Pipeline Design
- Data Visualization
- Customer Segmentation
- Predictive Analytics
- Supply Chain Analytics
- Business Intelligence

## Projects

| Project | Description | Tools |
|----------|-------------|--------|
| [Churn and Marketing Analytics](./Churn_and_Marketing_Analytics) | Customer churn prediction and customer segmentation | Python, SQL, Machine Learning |
| [ETL Architecture](./ETL_Architecture) | ETL pipeline design and architecture | SQL, ETL |
| [Olist Logistics Analysis](./Olist-Logistics-Analysis) | Logistics and delivery performance analysis | SQL, Power BI |
| [Olist Machine Learning](./Olist_Machine_Learning) | Predictive modeling with e-commerce data | Python, Machine Learning |
| [Power BI Sales Analysis](./PBI_Sales_Analysis) | Interactive sales and logistics dashboards | Power BI, SQL |



